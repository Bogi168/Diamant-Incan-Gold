...
def render_create_players(game_object):
    ...
    game_object.renderer.render(self = game_object.renderer, message = dashes_1())
    for player_num in range(players_amount):
        ...
        game_object.renderer.render(self = game_object.renderer, message=dashes_2())
...
def render_create_bots(game_object):
    ...
    if bots_amount != 0:
        for bot_num in range(bots_amount):
            game_object.renderer.render(self = game_object.renderer, message=dashes_3())
            ...
            game_object.renderer.render(self = game_object.renderer, message=dashes_4())

# Current round
def render_tell_round(game_object):
    game_object.renderer.render(self = game_object.renderer, message=tell_round(rounds = game_object.rounds))

# Tell the played cards
def render_tell_played_cards(game_object, cards_object):
    game_object.renderer.render(self = game_object.renderer, message=played_cards_text(cards_object = cards_object))

def render_tell_new_card(game_object, cards_object):
    game_object.renderer.render(self = game_object.renderer, message=tell_new_card(cards_object.new_card))

# Output depending on drawn card
def render_second_trap(game_object, cards_object):
    game_object.renderer.render(self = game_object.renderer, message=second_trap(cards_object.new_card))
    if game_object.rounds == 5 - 1:
        game_object.renderer.render(self = game_object.renderer, message=dashes_5())


# Tell situation
def render_tell_probability(game_object):
    game_object.renderer.render(self = game_object.renderer, message=tell_probability(game_object = game_object))

def render_tell_relics_on_way(game_object):
    if game_object.relics_on_way != 0:
        game_object.renderer.render(self = game_object.renderer, message=tell_relics_on_way(game_object = game_object))

def render_ask_player(game_object, current_player):
    game_object.renderer.render(self = game_object.renderer, message=tell_p_diamonds(game_object = game_object, current_player = current_player))
    ...
    game_object.renderer.render(self = game_object.renderer, message=dashes_6())
...
def render_bot_goes_home(game_object, current_bot):
    game_object.renderer.render(self = game_object.renderer, message=bot_goes_home(current_bot = current_bot))

# Bot stays inside
def render_bot_stays_inside(game_object, current_bot):
    game_object.renderer.render(self = game_object.renderer, message=bot_stays_inside(current_bot = current_bot))

# Present Bot's diamonds
def render_tell_b_diamonds(game_object, current_bot):
    game_object.renderer.render(self = game_object.renderer, message=tell_b_diamonds(game_object = game_object, current_bot = current_bot))
...
def render_tell_result(game_object):
    ...
    for explorer in game_object.explorers:
        game_object.renderer.render(self = game_object.renderer, message=tell_collected_diamonds(player_name = explorer.player_name, chest = explorer.chest))
    game_object.renderer.render(self = game_object.renderer, message=stars_1())
    game_object.renderer.render(self = game_object.renderer, message=tell_winners(game_object = game_object))
    game_object.renderer.render(self = game_object.renderer, message=stars_2())
...
def render_ask_again(game_object):
    ...
    if play_again == "Y" or play_again == "YES":
        game_object.renderer.render(self=game_object.renderer, message=new_line())
        game_object.reset_game()
    else:
        game_object.renderer.render(self = game_object.renderer, message=end_of_game())
