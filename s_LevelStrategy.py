...
class s_Level_4(LevelStrategy):
    def action(self):
        if ((self.game_object.prob_ev.calc_ev_next_dia_on_way(self.current_bot) - self.current_bot.guaranteed_diamonds) < 0
                and self.bool_diamonds_available()):
            self.current_bot.go_home()

class s_Level_5(LevelStrategy):
    def action(self):
        if (self.game_object.prob_ev.calc_ev_next(self.current_bot) - self.current_bot.pocket) < 0 and self.bool_diamonds_available():
            self.current_bot.go_home()

class s_Level_6(LevelStrategy):
    def action(self):
        self.game_object.prob_ev.calc_undiscovered_diamonds()
...
class s_Level_8(LevelStrategy):
    def action(self):
        self.game_object.prob_ev.calc_guaranteed_diamonds(self.current_bot)
